...
class AutheticationBloc extends Bloc<AutheticationEvent, AuthenticationState> {
  ...
  AutheticationBloc() : super(AuthenticationState()) {
    ...
    on<OnGetPuntosCorte>((event, emit) async {
      final puntosCorte = await geoDatasourceImpl.getDataPuntosCorte(1);
      emit(state.copyWith(
    ...
    on<OnEntrarLogin>((event, emit) async {
      add(const OnChangedProccessAuth(ProcesoAuthentication.procesando));

      final response =
          await geoDatasourceImpl.login(event.email, event.password);
      if (response.contains("OK")) {
        ...
        ShowLoadingCustom.showLoadingAuthSnackBar(event.context);
      }
    });
